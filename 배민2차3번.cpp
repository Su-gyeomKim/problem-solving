...
int main() {
	...
	//square s1, s2, s3;//1,2,3번 업소
					  //1번업소 = 2번(xy3~4) 3번(xy5~6)에 가려지는 나머지 부분 계산
	
	int h = y2 - y1, w = x2 - x1; //h=1번 업소 영역의 세로길이, w= 가로길이
	int ans = h*w;				  //1번 업소 기본 넓이
	...
	*/
	...
	if (y6>=y1) {//3번 위쪽이 1번 아래쪽보다 더 위라서,	h값은 y6-y1
		if (x1 <= x5 && x5<x2) ans -= (y6 - y1)*(x2 - x5);//1번 왼쪽이 3번 오른쪽 보다 작아,  w=x6-x1
		else if (x6 <= x2 && x6>=x1) ans -= (y6 - y1)*(x2 - x5);//3번 왼쪽이 1번 오른쪽 보다 작아,	w=x2-x5
		//이제 같을 때?
	}
	else if (y2 >= y5) {//1번 위쪽이 3번 아래쪽 보다 위라서,	h값은 y5-y2
		if (x1 <= x6) ans -= (y5 - y2)*(x6 - x1);//1번 왼쪽이 3번 오른쪽 보다 작아,  w=x6-x1
		else if (x5 <= x2) ans -= (y5 - y2)*(x2 - x5);//3번 왼쪽이 1번 오른쪽 보다 작아,	w=x2-x5
		//이제 같을 때?
	}
	if (y4>=y1) {
		if (x1 <= x4) ans -= (y4 - y1)*(x4 - x1);//1번 왼쪽이 3번 오른쪽 보다 작아,  w=x6-x1
		else if (x3 <= x2) ans -= (y4 - y1)*(x2 - x3);//3번 왼쪽이 1번 오른쪽 보다 작아,	w=x2-x5
	}
	else if (y2 >= y3) {//1번 위쪽이 2번 위쪽 보다 위라서,	h값은 y2-y3
		if (x1 <= x4) ans -= (y2 - y3)*(x4 - x1);//1번 왼쪽이 3번 오른쪽 보다 작아,  w=x6-x1
		else if (x5 <= x2) ans -= (y2 - y3)*(x2 - x5);//3번 왼쪽이 1번 오른쪽 보다 작아,	w=x2-x5										 //이제 같을 때?
	}
